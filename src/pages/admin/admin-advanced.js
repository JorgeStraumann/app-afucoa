...
let proposalRowsState = appMode === 'demo' ? adminProposals : [];
let proposalLoadFailed = false;
...
export function renderAdminProposals(){
  ...
  <section class="card admin-panel"><div class="admin-panel-head"><div><span class="eyebrow">Moderación</span><h2 id="proposal-admin-count">${proposalLoadFailed?'Propuestas no disponibles':`${proposalRowsState.length} propuestas`}</h2></div></div><div id="admin-proposal-list" class="admin-record-list">${proposalLoadFailed?proposalLoadError():proposalRows(proposalRowsState)}</div></section>`;
}
export async function bindAdminProposals(){
  ...
  try { proposalRowsState=await listAdminProposals();proposalLoadFailed=false; } catch(error){ proposalRowsState=[];proposalLoadFailed=true;console.error(error);flash('No se pudieron cargar las propuestas.',true); }
  let active='Todas';
  const paint=()=>{ const rows=active==='Todas'?proposalRowsState:proposalRowsState.filter(x=>x.status===active);root.innerHTML=proposalLoadFailed?proposalLoadError():proposalRows(rows);document.querySelector('#proposal-admin-count').textContent=proposalLoadFailed?'Propuestas no disponibles':`${rows.length} propuestas`;};
  ...
  root?.addEventListener('click',async e=>{const btn=e.target.closest('[data-proposal-action]');if(!btn)return; const id=btn.dataset.proposalId; const next=btn.dataset.proposalAction; let note=''; if(next==='respondida') note=window.prompt('Respuesta institucional','')||''; try{await moderateProposal(id,next,note);try{proposalRowsState=await listAdminProposals();proposalLoadFailed=false;paint();flash(appMode==='demo'?'Acción simulada.':'Propuesta actualizada.');}catch(error){proposalRowsState=[];proposalLoadFailed=true;paint();console.error(error);flash('La propuesta se actualizó, pero no se pudo recargar la lista.',true);}}catch(error){console.error(error);flash('No se pudo actualizar la propuesta.',true);}});
}
...
export async function bindAdminNotifications(){
  ...
  document.querySelector('#admin-notification-form')?.addEventListener('submit',async e=>{
    e.preventDefault(); const form=e.currentTarget; const fd=new FormData(form);
    try {
      const created = await createAdminNotification({ title:fd.get('title'), body:fd.get('body'), type:fd.get('type'), targetPath:fd.get('target_path'), priority:fd.get('priority') });
      notificationRowsState=await listAdminNotifications(); paint(); form.reset();
      const push=created.push;
      const detail=push?.status==='complete' || push?.status==='incidents'
        ? `Push: ${push.found} dispositivos encontrados, ${push.sent} enviados, ${push.failed} fallidos y ${push.deactivated} desactivados.${push.limited?' Envío parcial: quedan dispositivos por procesar.':''}`
        : push?.status==='disabled'?'Push deshabilitado.':push?.status==='not_configured'?'Push pendiente de configuración.':'Hubo incidencias al consultar el envío push.';
      flash(appMode==='demo'?'Notificación simulada.':`Notificación interna creada. ${detail}`);
      const note=document.querySelector('#notification-save-note');
      if(note){note.hidden=false;note.setAttribute('role','status');note.textContent=`Notificación interna creada. ${detail}`;}
...
function proposalLoadError(){return `<div class="empty-state admin-error-state" role="alert"><h3>Propuestas no disponibles</h3><p>No fue posible cargar los datos reales. Reintentá más tarde.</p></div>`;}
