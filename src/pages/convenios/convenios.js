...
export async function renderConvenios() {
  cachedAgreements = (await listAgreements()).map(normalize);
  ...
  return `
    <div class="page-title split"><div><span class="eyebrow">Beneficios para socios</span><h1>Convenios</h1><p>Encontrá beneficios y accedé a la acreditación necesaria desde la misma ficha.</p></div><button class="button secondary">♡ Mis favoritos</button></div>
...
export function bindConvenios() {
  ...
  const apply = () => {
    ...
    const filtered = cachedAgreements.filter(x => (category==='Todos'||x.category===category) && `${x.name} ${x.category} ${x.benefit} ${x.summary}`.toLocaleLowerCase('es').includes(q));
