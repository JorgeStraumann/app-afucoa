...
if (/sb_secret_[A-Za-z0-9_-]{16,}|\bservice_role\b|VAPID_PRIVATE_KEY|RESEND_API_KEY|BREVO_API_KEY/i.test(contents)) {
  fail('el artefacto contiene una clave privilegiada o un rol privilegiado.');
}
if (/-----BEGIN (?:[A-Z]+ )*PRIVATE KEY-----|\bre_[A-Za-z0-9]{24,}|\bxkeysib-[A-Za-z0-9_-]{16,}/.test(contents)) {
