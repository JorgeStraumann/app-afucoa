...
if ([string]::IsNullOrWhiteSpace($env:AFUCOA_DEV_SECRET_KEY)) {
  throw 'AFUCOA_DEV_SECRET_KEY no está disponible en esta sesión.'
}
...
try {
  ...
  $env:SUPABASE_SECRET_KEY = $env:AFUCOA_DEV_SECRET_KEY
  $env:AFUCOA_MFA_LIVE_PROJECT_REF = 'imiplnspvmsrsuikulwm'
  $env:AFUCOA_MFA_LIVE_CONFIRM = 'imiplnspvmsrsuikulwm'

  Push-Location -LiteralPath $repoRoot
  try {
    & $pnpmExecutable test:mfa-live
    if ($LASTEXITCODE -ne 0) { throw 'La validación MFA LIVE DEV falló.' }
  } finally {
    Pop-Location
  }
} finally {
