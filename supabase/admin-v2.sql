-- AFUCOA V2 — backend administrativo real
-- Incremento 10
-- Ejecutar DESPUÉS de schema-v2.sql y security-v2.sql en DESARROLLO.
-- Mantiene las mutaciones bajo RLS; este archivo no concede bypass al cliente.

-- Las políticas existentes ya limitan estas operaciones a public.is_admin().
-- Los GRANT son necesarios porque security-v2.sql dejó el cliente en lectura mínima.
...
-- Auditoría automática de mutaciones administrativas. La función se coloca en
-- un esquema no expuesto y fija search_path vacío, siguiendo las prácticas
-- recomendadas para SECURITY DEFINER.
...
begin
  ...
  if not exists (
    ...
    where p.id = v_actor and p.role in ('admin','superadmin')
  ) then
    return coalesce(new, old);
  end if;
...
-- RPC para cambiar estado/asignación de expedientes en una sola transacción.
...
-- RPC para moderación consistente de propuestas.
