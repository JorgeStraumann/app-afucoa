-- Follow-up: serialize first-use rate limits and bind codes to the exact Auth identity.
alter table public.password_recovery_codes add column if not exists auth_user_id uuid;
-- Old-format challenges are deliberately invalidated; never promote them to the new format.
update public.password_recovery_codes set invalidated_at = now()
 where auth_user_id is null and consumed_at is null and invalidated_at is null;
...
declare
  ...
  v_auth uuid;
begin
  if p_recovery_id is null or p_profile_id is null or p_code_hash is null
     or p_expires_at is null or p_request_ip_hash is null or p_code_hash !~ '^[0-9a-f]{64}$'
  ...
  select p.auth_user_id into v_auth from public.profiles p
   where p.id = p_profile_id and p.status = 'activo';
  ...
  insert into public.password_recovery_codes(
    id, profile_id, auth_user_id, code_hash, expires_at, max_attempts,
    delivery_status, request_ip_hash
  ) values (
    p_recovery_id, p_profile_id, v_auth, p_code_hash, p_expires_at, 5,
